...
If you're running multiple AI agents in production, you're asking the same question every team asks at scale: "which of my agents is producing good work, and which is silently producing garbage?" nthlayer-measure answers that question. Point it at your agents, and it tells you which ones are reliable, which ones are degrading, and which ones need to be reined in before they cause damage.

nthlayer-measure evaluates AI agent output quality, tracks per-agent quality trends over rolling windows, detects quality degradation, measures its own accuracy through self-calibration, and governs agent autonomy based on measured performance. It works with any agent system (not just a specific framework) and any model provider (swap Claude for Gemini or a local model and the transport doesn't change, only the judgment quality changes, which is itself measurable).
...
```
Agent Output ──▶ nthlayer-measure ──▶ Quality Scores + Trends + Alerts
                   │
...
nthlayer-measure receives agent output (via adapters for different agent systems), routes it to a model for quality evaluation, persists the scores, tracks trends over time, and detects when quality degrades. The entire pipeline follows [Zero Framework Cognition](ZFC.md): the code handles receiving, routing, persisting, and alerting (transport), while the model handles the actual quality evaluation (judgment).
...
# Run calibration report (how accurate is nthlayer-measure's own judgment?)
...
# Calibration: how well does nthlayer-measure agree with human corrections?
...
# List recent human overrides (corrections to nthlayer-measure scores)
...
nthlayer-measure is built on [Zero Framework Cognition](ZFC.md), which draws a hard line between transport and judgment.
...
This separation means nthlayer-measure is model-agnostic by design. Swap the evaluation model and the transport layer is unchanged. The quality of judgment changes, and that change is itself measurable through the self-calibration loop.

---

## Signals & Judgment SLOs

nthlayer-measure tracks two categories of signals: **quality signals** (about the agents it monitors) and **calibration signals** (about its own judgment accuracy).
...
| **Reversal rate** | Fraction of evaluations that were later corrected by a human override. A reversal rate of 0.05 means 5% of nthlayer-measure's judgments were wrong enough for a human to step in. |
| **Cost per evaluation** | Token spend per evaluation, broken down by agent |

### Calibration signals (nthlayer-measure judging itself)

When a human submits an override (correcting an nthlayer-measure score), that override becomes ground truth. nthlayer-measure measures how well its judgments align with human corrections:

| Signal | Definition | Good value |
|--------|-----------|------------|
| **Reversal rate** | Overridden evaluations / total evaluations | < 0.05 (5%) |
| **False accept rate** | Of outputs where humans scored lower than nthlayer-measure, how many did nthlayer-measure score above the passing threshold? Measures how often nthlayer-measure lets bad work through. | < 0.02 (2%) |
| **Precision** | Of outputs nthlayer-measure flagged as low quality, what fraction did humans agree with (no upward override)? High precision = few false alarms. | > 0.90 |
| **Recall** | Of outputs humans corrected downward, what fraction did nthlayer-measure also flag as low quality? High recall = nthlayer-measure catches what humans catch. | > 0.85 |
| **MAE** | Mean absolute error between original nthlayer-measure scores and human-corrected scores, per dimension. | < 0.10 |

### OpenSRM judgment SLO targets

When an agent has an [OpenSRM manifest](#integration-with-opensrm), nthlayer-measure checks calibration signals against declared targets:
...
Without a manifest, nthlayer-measure still computes all signals — it just doesn't have compliance targets to check against.
...
Every `check()` call emits a `gen_ai.calibration.report` OTel event with all metrics, feeding into NthLayer dashboards and nthlayer-correlate correlation.

---

## Governance

nthlayer-measure subsumes the role of a reliability governor. Rather than having two separate systems that watch agent quality, nthlayer-measure both measures quality and acts on those measurements.
...
- An agent that is expensive and low-quality gets constrained faster than one that is cheap and low-quality
...
nthlayer-measure reads judgment SLO thresholds from [OpenSRM](https://github.com/rsionnach/opensrm) manifests when they're available. An agent's manifest declares its quality targets:
...
nthlayer-measure also works with simple configuration files for teams that don't use OpenSRM. The manifest integration is additive, not required.

---

## Adapters

nthlayer-measure connects to different agent systems through adapters. Each adapter translates a specific system's output format into nthlayer-measure's evaluation pipeline.
...
Building a custom adapter is straightforward: implement the input interface to receive agent output, and nthlayer-measure handles evaluation, scoring, trending, and governance from there.

---

## OpenSRM Ecosystem

nthlayer-measure is one component in the OpenSRM ecosystem. Each component solves a complete problem independently, and they compose when used together through shared OpenSRM manifests and OTel telemetry conventions.
...
**How nthlayer-measure fits in:**

- Every evaluation produces a **verdict** stored in the shared Verdict Store. Self-calibration queries `verdict.accuracy()` to measure nthlayer-measure's own judgment quality. Human overrides call `verdict.resolve()` — the same mechanism used across all ecosystem components.
- **Quality verdicts** flow to [nthlayer-correlate](https://github.com/rsionnach/nthlayer-correlate) as events for correlation with other signals (deployments, model version changes), and to [nthlayer-respond](https://github.com/rsionnach/nthlayer-respond) during incident response. OTel side-effects of verdict operations feed [NthLayer](https://github.com/rsionnach/nthlayer)-generated dashboards.
- **Governance decisions** adjust agent autonomy across the ecosystem, including nthlayer-respond's incident response agents and nthlayer-correlate's correlation agent
- **Cost tracking data** feeds into NthLayer-generated dashboards so operators see quality and cost together

Each component works alone. Someone who just needs agent quality measurement adopts nthlayer-measure without needing NthLayer, nthlayer-correlate, or nthlayer-respond.
...
The nthlayer-measure concept was proven inside GasTown as the Guardian, a Deacon plugin that scores per-worker output quality in the merge pipeline ([PR #2263](https://github.com/nicholasgastown/nicholasgastown/pull/2263), merged by Steve Yegge). The Guardian demonstrated that automated quality measurement with self-calibration works in practice at production scale. nthlayer-measure extracts this pattern into a universal tool that any multi-agent system can use, regardless of the underlying agent framework or model provider.

---

## Status

nthlayer-measure is fully implemented:
